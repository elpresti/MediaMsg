<?php
    ...
    if (isset($_FILES["audio-blob"])) {
        $uploadDirectory = '/var/www/mediamsg/tmp/'.$_POST["filename"].'.wav';
        if (!move_uploaded_file($_FILES["audio-blob"]["tmp_name"], $uploadDirectory)) {
            ...
            if (isset($_FILES["video-blob"])) {
                $uploadDirectory = '/var/www/mediamsg/tmp/'.$_POST["filename"].'.webm';
                if (!move_uploaded_file($_FILES["video-blob"]["tmp_name"], $uploadDirectory)) {
                    echo("Problem writing video file to disk!");
                }
                else {
                    $audioFile = '/var/www/mediamsg/tmp/'.$_POST["filename"].'.wav';
                    $videoFile = '/var/www/mediamsg/tmp/'.$_POST["filename"].'.webm';
                    
                    //$mergedFile = '/var/www/mediamsg/tmp/'.$_POST["filename"].'-merged.webm';
                    $mergedFile = '/var/www/mediamsg/tmp/'.$_POST["filename"].'-merged.mp4';
                    ...
                    if(!strrpos($CurrOS, "Windows")) {
                        //$cmd = '-i '.$audioFile.' -i '.$videoFile.' -map 0:0 -map 1:0  -strict -2 '.$mergedFile;
                        //$cmd = '-i '.$audioFile.' -itsoffset 1.8 -i '.$videoFile.' -map 0:0 -map 1:0  -c:v libx264 -vf fps=25  -c:a aac -strict -2 '.$mergedFile;
                        $cmd = '-i '.$audioFile.' -i '.$videoFile.' -map 0:0 -map 1:0  -c:v libx264 -vf fps=25  -c:a aac -strict -2 '.$mergedFile;
                    }
                    else {
